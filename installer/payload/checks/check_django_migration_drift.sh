...
printf '$ %s manage.py makemigrations --check --dry-run\n' "$python_bin"
if (cd "$root" && "$python_bin" manage.py makemigrations --check --dry-run); then
  printf 'K_CHECK_STATUS=ok\n'
  exit 0
fi

printf 'K_CHECK_STATUS=fail\n'
printf 'K_CHECK_REASON=missing migrations detected\n'
exit 1
