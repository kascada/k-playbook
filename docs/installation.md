...
If `VERSION` changed and the running binary is **older** than it, the new version requires another binary. The service then installs it on the spot: it runs the clone's bootstrap, `k-playbook/bin/install`, which downloads the release asset for this `VERSION`, verifies it against `SHA256SUMS`, and atomically replaces `~/.local/bin/k-playbook`. It then hands over its runtime file to a new service started from that file, and the open page follows to the new address, where the header shows the new version. Nothing is downloaded on startup or during a check: only an explicit click installs. If the binary is equally old or newer, the usual case in the development repository where `make dev-install` installs it before the clone, nothing is installed and the service continues. The comparison is semantic, and only "older" triggers it.

If the installation fails -- no network, checksum mismatch, no `curl` or `wget` -- the old service keeps running and the page names the error, the output, and the command to catch up by hand: `make -C k-playbook install`, or without make `k-playbook/bin/install`. The same holds if `k-playbook` in the `PATH` does not resolve to `~/.local/bin/k-playbook`: then there is no button, only a note with both paths. Installing anyway would put a new binary where the next call to `k-playbook` never looks; that call would start the old program again, and its startup would stop the newly started service. Remove the other file or move `~/.local/bin` to the front of the `PATH`, then run the bootstrap in the terminal. Afterwards, `k-playbook` starts the new version. After a manual `git pull` or `make -C k-playbook installer-update`, an old service initially keeps running; it is detected and replaced only when the next call to `k-playbook` comes from a newly installed binary. The binary file itself is compared, not only its version, so a newly built binary with the same `VERSION` also replaces the old service. If only commands, rules, or recipes are new, `VERSION` does not change: the service continues, `Reload` in the interface loads the version, and restarting the assistant is enough.
...
**One version, one binary.** `VERSION` at the root of the clone names the release whose assets belong to this clone version; `SHA256SUMS` beside it contains their checksums. Commits to rules, reviews, commands, or docs change neither file. If `VERSION` changes during an update and the running binary is older than it, the new version requires another binary. It is installed through the bootstrap and only on an explicit click; the program never downloads anything on startup, during a check, or beside an installation of its own. There is no download code in the Go program: the interface calls the clone's `bin/install` and afterwards verifies the installed file's sha256 against `SHA256SUMS` before it restarts from it.
...
**The header shows an older version than the clone.** The running program is older than `VERSION` in the clone; `k-playbook` says so on startup, and the interface offers `Update program (v0.9.3 -> v0.9.4)`. One click installs through the bootstrap and restarts the service from the new program. If there is a note with two paths instead of a button, `k-playbook` in the `PATH` resolves to a different file than `~/.local/bin/k-playbook`; resolve that first, otherwise the next call would start the old program again. `k-playbook version` prints which version a file carries.
