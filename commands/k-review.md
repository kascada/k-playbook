...
`/k-review` does not guess project paths. The project must have `K-PLAYBOOK.yaml` configured by the k-playbook Installer.
...
- If `PROJECT_REVIEWS_DIR` is missing: abort and tell the user to open the k-playbook Installer and use `Vervollstaendigen` in the project block.
...
2. Ergebnis schreiben. If `RESULT_DIR` is unset, abort and ask the user to open the k-playbook Installer and use `Vervollstaendigen` in the project block.
