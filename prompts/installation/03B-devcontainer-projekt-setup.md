...
6. Stelle sicher, dass das Projekt im k-playbook Installer eingebunden ist; dort werden `K-PLAYBOOK.yaml` und die projektlokale Struktur angelegt bzw. vervollstaendigt. Fuehre danach im Projektroot `/k-setup` aus, um die Kernkonfiguration zu pruefen oder zu aktualisieren. Wenn du Slash-Commands nicht selbst ausloesen kannst, lies `~/dev/k-playbook/commands/k-setup.md` direkt und fuehre die dort beschriebenen Schritte aus.
